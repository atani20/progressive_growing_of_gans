...
class TFRecordDataset:
    def __init__(self,
        ...
        with tf.compat.v1.name_scope('Dataset'), tf.device('/cpu:0'):
            ...
            self._tf_iterator = tf.compat.v1.data.Iterator.from_structure(tf.compat.v1.data.get_output_types(self._tf_datasets[0]),
                                                                          tf.compat.v1.data.get_output_shapes(self._tf_datasets[0]))
...
class SyntheticDataset:
    def __init__(self, resolution=512, num_channels=2, dtype='uint16', dynamic_range=[0,4095], label_size=0, label_dtype='float64'):
